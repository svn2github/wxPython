...
"""
:class:`~xlsgrid.XLSGrid` is a class based on `wx.grid.Grid` that can be used to faithfully
reproduce the appearance of a Microsoft Excel spreadsheet (one worksheet per
every instance of :class:`~xlsgrid.XLSGrid`).
...
:class:`~xlsgrid.XLSGrid` is a completely owner-drawn control, and it relies on the power of
...
:note:
 ...
 :class:`~xlsgrid.XLSGrid` **requires** the `xlrd` package from:
...
:note:
 ...
 worksheet in your instance of :class:`~xlsgrid.XLSGrid`.

:warning:

 If Mark Hammonds' `pywin32` package is not available, the formatting
 capabilities of :class:`~xlsgrid.XLSGrid` are severely limited; for instance, you won't
 probably get the exact WYSIWYG between the Excel spreadsheet and :class:`~xlsgrid.XLSGrid`.


:warning:

 :class:`~xlsgrid.XLSGrid` can only read Excel `.xls` files, not the newer versions
...
* Cell text: support for all kind of fonts (except strikethrough, but this is
  ...
  :class:`~xlsgrid.XLSGrid` supports the following features found in Excel:
...
* Cell text appearance: if you are using Mark Hammonds' `pywin32` package, the
  text displayed in the :class:`~xlsgrid.XLSGrid` cells has exactly the same appearance as in
...
* Cell hyperlinks: starting from version 0.7.2 (SVN), `xlrd` is capable of
  ...
  in :class:`~xlsgrid.XLSGrid` with a cursor changing and a tooltip on that cell.

* Cell merging: merged cells in the Excel spreadsheet will be correctly handled
  by :class:`~xlsgrid.XLSGrid`.

* Columns and rows sizes: :class:`~xlsgrid.XLSGrid` calculates the correct rows and columns
...
1. :class:`~xlsgrid.XLSGrid` is sufficiently efficient and fast for reasonably small Excel files.
   There might be some improvement to be made in the code to make it work with
   bigger files and in a faster way;
...
:class:`~xlsgrid.XLSGrid` has been tested on the following platforms:
...
:class:`~xlsgrid.XLSGrid` is distributed under the wxPython license. 
...
#        else:
    ...
    def CreateFormat(self, format, cell, datemode):
        ...
        :warning: The formatting applied by this method is severely limited; for
         ...
         spreadsheet and :class:`~xlsgrid.XLSGrid`.         
        """
    ...
    def CombineAttr(self, attr):
        """
        Combines the input attribute `attr` with the features of the :class:`~xlsgrid.XLSText` class.
...
class XLSRichText(XLSText):
    ...
    to what :class:`~xlsgrid.XLSText` does, this class attempts to handle cells with rich text
    content.
    """
    ...
    def __init__(self, book, cell, xf_index, display_text=None, hyperlink=None, rich_text=None, default_width=10):
        ...
        :param `hyperlink`: if this cell contains a hyperlink, it will be displayed
         accordingly;
        :param `rich_text`: if this cell contains text in rich text format, :class:`~xlsgrid.XLSGrid`
        ...
        :note: If you are using version 0.7.1 or lower for `xlrd`, this class will
         note be used by :class:`~xlsgrid.XLSGrid`.
...
class XLSBackground(object):
    ...
    def CombineAttr(self, attr):
        """
        Combines the input attribute `attr` with the features of the :class:`~xlsgrid.XLSBackground` class.
...
class XLSCell(object):
    """
    This is a class which holds information about a single cell in :class:`~xlsgrid.XLSGrid`.
    ...
    def SetupCell(self, book, cell, xf_index, xls_text, xls_comment, hyperlink, rich_text, default_width, default_colour):
        """
        Actually sets up the :class:`~xlsgrid.XLSCell` class. This is an auxiliary method to
    ...
    def SetValue(self, value):
        ...
        :note: This method is currently unused as everything is handled inside the :class:`~xlsgrid.XLSText` class.
    ...
    def GetComment(self):
        ...
        :returns: an instance of :class:`~xlsgrid.XLSComment`.
...
class XLSRenderer(gridlib.PyGridCellRenderer):
    ...
    def __init__(self, cell):
        ...
        :param `cell`: an instance of :class:`~xlsgrid.XLSCell`.        
        """
...
class XLSTable(gridlib.PyGridTableBase):
    ...
    def __init__(self, grid, cells, rows, cols):
        ...
        :param `cells`: a Python dictionary. For every key `(row, col)`, the
         corresponding value is an instance of :class:`~xlsgrid.XLSCell`;
...
class XLSGrid(gridlib.Grid):
    ...
    def InstallGridHint(self):
        """
        Auxiliary method used to bind a ``wx.EVT_MOTION`` event to :class:`~xlsgrid.XLSGrid`.
        """
        ...
        def OnMouseMotion(event):
            """
            Handles a the ``wx.EVT_MOTION`` events for :class:`~xlsgrid.XLSGrid`.
    ...
    def FormatCell(self, book, sheet, row, col, display_texts, comments, hyperlink, rich_text, default_width, default_colour):
        """
        Processes the creation of a single cell (an instance of :class:`~xlsgrid.XLSCell`).
...
class TransientPopup(STT.SuperToolTip):
    """
    This is a sublass of :class:`~supertooltip.SuperToolTip` and it is used to display a
    ...
    def __init__(self, grid_window, comment, position):
        ...
        :param `comment`: an instance of :class:`~xlsgrid.XLSComment`, containing the
